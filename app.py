...
st.title("📊 KOSPI 200 전략 모니터 (v4.0)")
...
def get_rebalance_days(idx):
    r_dates = []
    for _, group in idx.to_series().groupby(pd.Grouper(freq='ME')):
        if group.empty: continue
        d1 = group.iloc[0].replace(day=1)
        # 2nd Thu + 1 day (Friday)
        first_thu = d1 + timedelta(days=((3 - d1.weekday() + 7) % 7))
        sec_thu = first_thu + timedelta(days=7)
        fri = sec_thu + timedelta(days=1)
        if fri in idx: r_dates.append(fri)
    return r_dates
...
if not df.empty:
    ...
    for i, r_day in enumerate(re_days):
        ...
        if not tmp.empty:
            ...
            # 목표선 및 달성 마커
            tp = s_idx * 1.05
            fig.add_shape(type="line", x0=tmp.index[0], x1=tmp.index[-1], y0=tp, y1=tp, line=dict(color="Red", width=1, dash="dot"))
            ...
            if not hits.empty:
                ...
                for dt, row in hits.iterrows():
                    is_t = dt.date() == now.date()
                    st_val = "✅ 발송 성공" if dt.date() >= SYSTEM_START_DATE and not is_t else "⚪ 미실행"
                    if is_t and now.hour < 15: st_val = "⏳ 발송 예정"
