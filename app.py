...
st.title("📊 KOSPI 200 전략 모니터 (v7.0 Master)")
...
# 리밸런싱일(매월 두 번째 목요일 다음날 금요일) 계산 함수
def get_rebalance_days(idx):
    ...
    for _, group in groups:
        ...
        # 두 번째 목요일 다음 날은 금요일
        target_fri = first_thu + timedelta(days=8)
        
        # 실제 데이터 인덱스 중 가장 가까운 날짜 매칭
        # DatetimeIndex에서 직접 argmin 사용 (가장 안전한 방식)
        closest_idx = (idx - target_fri).abs().argmin()
...
if not df.empty:
    ...
    profit_pct = ((curr_p - base_p) / base_p) * 100

    # 3. 상단 지표 레이아웃 (순서: 기준 -> 목표 -> 현재 -> 기준지수대비)
    c1, c2, c3, c4 = st.columns(4)
    
    c1.metric("기준 지수", f"{base_p:.2f}", help=f"최근 기준일: {last_re.strftime('%Y-%m-%d')}")
    
    t_diff = target_p - curr_p
    if t_diff <= 0:
        t_label, t_color = "🔥 초과 상태 중", "normal"
    else:
        t_label, t_color = f"{t_diff:.2f} 남음", "inverse"
    c2.metric("목표 지수 (5%)", f"{target_p:.2f}", delta=t_label, delta_color=t_color)
    
    c3.metric("현재 지수", f"{curr_p:.2f}", delta=f"{curr_p - base_p:+.2f}")
    
    c4.metric("기준지수대비", f"{profit_pct:.2f}%")
    ...
    for i, r_day in enumerate(re_days):
        ...
        if not tmp.empty:
            ...
            # 분배락 표시 로직 (가장 가까운 거래일 매칭)
            if not dividends.empty:
                seg_div = dividends[(dividends.index >= tmp.index[0]) & (dividends.index <= tmp.index[-1])]
                for d_dt, _ in seg_div.items():
                    # tmp.index(DatetimeIndex)에서 d_dt(Timestamp) 사이의 최소 거리 인덱스 찾기
                    closest_dt_idx = (tmp.index - d_dt).abs().argmin()
    ...
    with l_col:
        ...
        if logs:
            st.dataframe(pd.DataFrame(logs).sort_values("날짜", ascending=False), 
    ...
    with d_col:
        ...
        if not dividends.empty:
            ...
            d_df.columns = ['배당락일', '원']
            st.dataframe(d_df.sort_values('배당락일', ascending=False).head(10), 
                         use_container_width=True, hide_index=True)
