...
if not df.empty:
    ...
    # 4. 차트 시각화
    st.subheader("📈 지수 vs ETF 수익률 동기화 (영점 조정 및 분배락)")
    fig = go.Figure()
    logs, now_d = [], datetime.now().date()
    ...
    for i, r_day in enumerate(re_days):
        ...
        if not tmp.empty:
            ...
            # 목표선 및 삼각형
            tp = s_idx * 1.05
            fig.add_shape(type="line", x0=tmp.index[0], x1=tmp.index[-1], y0=tp, y1=tp, line=dict(color="Red", width=1.2, dash="dot"))
            ...
            if not hits.empty:
                ...
                for dt, row in hits.iterrows():
                    dt_date = dt.date()
                    is_t = dt_date == now_d
                    st_val = "✅ 발송 성공" if dt_date >= SYSTEM_START_DATE and not is_t else "⚪ 미실행"
                    if is_t and datetime.now().hour < 15: st_val = "⏳ 발송 예정"
